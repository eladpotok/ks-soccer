...
function ParticipantsList(props) {
    ...
    const players = playersContext.getPlayersAccordingToLevel(props.levelType + 'Level');
    const levelDisplay = props.levelType === GROUP_TYPE.low ? 'LOWER' : 'HIGHER';
    ...
    function getNumberOfPlayersLabel() {
        const number = getPlayersLength();
        if (number === 0) {
            return (<div style={{ color: 'white', display: 'flex', alignItems: 'center', }}>No Players Yet</div>)
        }

        return (<div style={{ color: 'white' }}>Players: {number}/20</div>)
    }

    const sortbyStars = () => {
        players.sort(function (a, b) { return b.stars - a.stars });
        updateByLevels(players);
    };
    ...
    function updateByLevels(players) {
        if(props.levelType === GROUP_TYPE.low) {
            playersContext.setPlayersByLevels(  { lowLevel: players, highLevel: playersContext.playersByLevel.highLevel  }  );
        }  
        else {
            playersContext.setPlayersByLevels(  { highLevel: players, lowLevel: playersContext.playersByLevel.lowLevel  }  );
        }
    }
        ...
        <>
            <article className="leaderboard">
                <header style={{height: '170px'}}>
                    ...
                    <svg version="1.1" id="Layer_1" x="0px" y="0px" viewBox="0 0 511.999 511.999" className="leaderboard__icon">
                        <g>
                            <g>
                  ...
                  c1.727,3.497,5.062,5.921,8.922,6.482l18.162,2.64L278.064,146.405z" />
                    ...
                    <h1 className="leaderboard__title"><span className="leaderboard__title--top">{levelDisplay}</span><span className="leaderboard__title--bottom">{<div>{getNumberOfPlayersLabel()}</div>}</span></h1>
                    <h1 className="leaderboard__title">
                        <span className="leaderboard__title--sort">
                            <Button onClick={sortbyAB} className='sort-ab-button' icon={<AiOutlineSortAscending/>} ></Button>
                            <Button onClick={sortbyStars} className='sort-ab-button' icon={<BsSortNumericDownAlt />} ></Button>
                        </span>
                    </h1>

                </header>

                <main class="leaderboard__profiles" style={{
                    maxHeight: '550px',
                    overflowY: 'scroll'
                }}>
                    ...
                    {players && players.map((player) => (
                        <>
                            <AdminWrapper>
                                <article className="leaderboard__profile__admin">
                                    <span className="leaderboard__name" style={{ marginTop: '7px' }} >
                                        <Popconfirm title="Do you want to remove this player?" onConfirm={() => removePlayerHandler(player.id)}>
                                            <GoDiffRemoved style={{ color: '#c2256e' }} />
                                        </Popconfirm>

                                    </span>
                                    <span className="leaderboard__name" style={{ marginTop: '7px' }} >
                                        <Popconfirm title="Do you want to move this player?" onConfirm={() => onMoveLevelHandler(player.id)}>
                                            {props.levelType === GROUP_TYPE.low && <BsArrowRightSquare />}
                                            {props.levelType !== GROUP_TYPE.low && <BsArrowLeftSquare />}
                                        </Popconfirm>
                                    </span>
                                    <span className="leaderboard__name">{player.name}</span>
                                    <span className="leaderboard__value">{player.stars}<Stars stars={[1]} /></span>
                                </article>
